...
def wrap_with_click_outside(window, popup_width, center=False):
    ...
    If center=True, the popup is horizontally centered on the active monitor
    instead of being anchored under the cursor. Vertical placement is the same
    as the standard popups (top, just under the bar) so all popups share a
    consistent gap from waybar.
    """
