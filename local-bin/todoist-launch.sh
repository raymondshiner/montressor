...
if [ -n "$JV" ]; then
    ...
    hyprctl dispatch exec "[workspace $WS silent; tile] kitty --class=jarvis-todoist --title=JarvisTodoist --config=$HOME/.config/kitty/jarvis.conf -e env AGENT_TASK_FILE=$TF JARVIS_MCP=todoist $HOME/.local/bin/jarvis"
